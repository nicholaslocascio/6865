...
int main(int argc, char** argv)
{
    ...
    // file. You can see these files to understand the loop nests produced by the
    ...
    // Eachedule will create a HTML file that shows its generated pseudo-code
    // This should help you undersand the shceduling opeations.
    ...
    ////// First we observe the default ordering.
    {
        ...
        gradient.compile_to_simplified_lowered_stmt(out_dir+gradient.name()+".html", out_buffer, HTML);
    ...
    {
        Func gradient("tut5_gradient_split_5x4");
        gradient(x, y) = x + y;
        ...
        cout << "Evaluating gradient over a 5x4 box with x split by two" << endl;
        gradient.compile_to_simplified_lowered_stmt(out_dir+gradient.name()+".html", out_buffer, HTML);
        gradient.realize(out_buffer);
    }
    ...
    // Are you ready? We're going to use all of the features above now.
    {
        ...
        gradient_fast.compile_to_simplified_lowered_stmt(out_dir+gradient_fast.name()+".html", out, HTML);
        gradient_fast.realize(out);
