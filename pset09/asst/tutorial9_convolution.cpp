...
void boxBlur(Image<uint8_t> input, int indexOfBlur=0)
{
    ...
    switch(indexOfBlur)
    {
        ...
        case 2:
            cout << "Box blur with the *sum* sugar " << endl;
    ...
    std::string file = "Output/tut9_" + blur.name()+Halide::Internal::int_to_string(indexOfBlur)+".html";

    blur.compile_jit();

    Buffer buffer(type_of<float>(), input.width(), input.height(), input.channels());
    blur.compile_to_simplified_lowered_stmt(file, buffer, HTML);
